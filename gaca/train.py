...
from ablations.models import SingleModality, MeanVectorGate, AttentionConcat, CrossAttention, MeanConcat
...
DEFAULT_FEATURE_ROOT = REPOSITORY_ROOT / "data" / "embeddings" / "feature_store"
...
def save_or_load_split(
    dataset, output_dir, validation_ratio, seed, split_file=None
):
    split_path = Path(split_file) if split_file else output_dir / "fixed_fit_validation_split.csv"
    if split_file is not None and not split_path.exists():
...
def evaluate_test(model, dataset, loader, device, use_amp):
    ...
    for batch in loader:
        ...
        with torch.autocast(
            ...
            if isinstance(model, GaCA):
                logits, gates = model.forward_with_gate(
                    sequence, structure, seq_mask, str_mask
                )
    ...
    full_probabilities = np.concatenate(probability_batches, axis=0)
    full_gates = np.concatenate(gate_batches, axis=0) if gate_batches else None
    return metrics, result, full_probabilities, full_gates
...
def run_model(
    ...
    for dataset_name, dataset in test_datasets.items():
        ...
        metrics, predictions, probabilities, gates = evaluate_test(
        ...
        np.savez_compressed(
            model_dir / f"probabilities_{dataset_name}.npz",
            probabilities=probabilities,
            true_indices=dataset.labels,
            protein_ids=np.asarray(dataset.ids, dtype=str),
        )
        if gates is not None:
            np.save(model_dir / f"gate_vectors_{dataset_name}.npy", gates)
        print(
...
def main():
    ...
    if not args.skip_protocol_check:
        validate_final_protocol(train_dataset, test_datasets)
    ...
    if not args.skip_protocol_check:
        validate_partitions(train_dataset.labels, fitting_indices, validation_indices)
