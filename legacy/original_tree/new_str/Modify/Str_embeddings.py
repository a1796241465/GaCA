...
TASKS = [
    ...
    {
        "name": "Test Set <30% (高难测试集)",
        "pdb_dir": "/home/lihaotian/new_ec/test_structures/",
        "output_path": "./test_structure_embeddings_esm_if.pkl"
    },
    {
        "name": "Test Set 30-50% (中等测试集)",
...
def extract_structure_embedding(model, coords_np):
    ...
    Input:
        coords_np: (L, 3, 3)
    Output:
        residue_repr: (L, 512) 【已移除 Mean Pooling，输出二维残基矩阵】
    """
    ...
    with torch.no_grad():
        ...
        residue_repr = residue_repr / (residue_repr.norm(dim=-1, keepdim=True) + 1e-6)


    return residue_repr.cpu().numpy().astype(np.float16)
...
def process_directory(model, task_conf):
    ...
    print("   ➤ 【已修改】保留残基级二维矩阵 (L, 512) + 半精度防爆内存")
