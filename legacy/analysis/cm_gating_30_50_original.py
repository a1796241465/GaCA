...
MODEL_PATH = '/home/lihaotian/new_ec/new_str/test_30-50/best_GACMA_final.pth'
...
def plot_gating_dist_clean(g_values):
    ...
    plt.title("Distribution of Gating Weights (30-50%)", fontweight='bold', fontsize=14, pad=15)
    ...
    plt.savefig("Fig_Advanced_Gating_30-50.pdf", dpi=300)
    print("✅ 门控分布图已保存: Fig_Advanced_Gating_30-50.pdf")
...
def plot_tsne(features_before, features_after, labels_l1):
    ...
    axes[0].set_title("Raw ESM-2 Features (30-50%)", fontweight='bold');
    axes[0].axis('off')

    scatter2 = axes[1].scatter(emb_after[:, 0], emb_after[:, 1], c=c_ids, cmap='tab10', s=15, alpha=0.6)


    axes[1].set_title("GACMA Fused Features (30-50%)", fontweight='bold');
    axes[1].axis('off')
    ...
    plt.savefig("Fig_Advanced_TSNE_30-50.pdf", dpi=300, bbox_inches='tight')
...
def plot_confusion_matrix(y_true_l1, y_pred_l1):
    ...
    plt.title("Confusion Matrix (30-50%)", fontweight='bold', pad=15)

    plt.tight_layout()

    plt.savefig("Fig_Advanced_CM_30-50.pdf", dpi=300)
